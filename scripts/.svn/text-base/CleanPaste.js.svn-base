...
CleanPaste.prototype.CleanHTML = function(html) {
    ...
    html = html.replace(/(class|style|type|start|id)=("(.*?)"|(\w*))/gi, ''); // Unwanted sttributes
    html = html.replace(/<style(.*?)style>/gi, '');   // Style tags
    html = html.replace(/<script(.*?)script>/gi, ''); // Script tags
    html = html.replace(/<!--(.*?)-->/gi, '');        // HTML comments

    // Added by Abdullah ------
    // Converts all headings and <b> to <strong> tags.
    html = html.replace(/<h\d\s*>/ig,"<strong>");
    html = html.replace(/<\/h\d\s*>/ig,"</strong><br />");
    html = html.replace(/<b\s*>/ig,"<strong>");
    html = html.replace(/<\/b\s*>/ig,"</strong>");
    // Converts <i> to <em> tags.
    html = html.replace(/<i\s*>/ig,"<em>");
    html = html.replace(/<\/i\s*>/ig,"</em>");
    // Added by Abdullah ------
